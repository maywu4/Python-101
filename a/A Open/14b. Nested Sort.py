...
# Write a function, sort_teachers_by_classroom_capacity, that sorts the results in ascending order based on their classroom's capacity and returns a list of just the teacher's names in that sorted order.
...
def sort_teachers_by_classroom_capacity(card_list):
    sortedClass = sorted(card_list, key= lambda teacher: teacher['classroom']['capacity'])
    teacherNames = map(lambda teach: teach['name'], sortedClass)
    return list(teacherNames)
